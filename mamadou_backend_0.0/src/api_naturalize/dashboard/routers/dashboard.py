from fastapi import APIRouter, HTTPException, status, File, UploadFile, Form, Request, Query, Depends
...
from api_naturalize.dashboard.schemas.dashboard import (
    ...
    UserGrowthResponse,
    UserStatusFilter, 
    QuestionStatisticsWithCourseResponse
)
...
async def create_course(
    ...
    # Construct image URL in the response for each type of request
...
# GET user activity statistics --- ***SPARKS EDITION***
...
async def get_user_activity_statistics(days: int = 30):
    ...
    # Users updated in the period ***(Non-admin only)***
    ...
    try:
        ...
        non_admin_users = await UserModel.find(
            UserModel.id.in_(recent_progress_users),
            UserModel.role == UserRole.USER
        ).count()
        
        users_with_recent_progress = non_admin_users
    except:
...
@router.patch("/user/status/change/{user_id}", status_code=status.HTTP_200_OK)
async def change_user_status(
    user_id: str,
    new_status: str,
    admin: dict = Depends(get_user_info)  # Require admin auth
):
    """
    Change user account status
    Accepts: active, inactive, suspended (case-insensitive)
    """
    # Validate admin role
    admin_user = await UserModel.get(admin["user_id"])
    ...
    status_mapping = {
        ...
        "suspend": AccountStatus.SUSPENDED,  # Legacy support
    }
    ...
    if status_lower not in status_mapping:
        raise HTTPException(
            ...
            detail=f"Invalid status: {new_status}. Use: active, inactive, or suspended"
        )
    ...
    user.account_status = status_mapping[status_lower]
    ...
    try:
        notification = NotificationModel(
            ...
            description=f"Your account status has been changed to {status_mapping[status_lower].value}",
...
@router.get("/user/all/filter/{filter_value}", status_code=status.HTTP_200_OK)
async def get_users_by_filter(
    ...
    """
    Filter users by status or plan
    Supports: active, inactive, suspended, free, basic, premium
    """
    ...
    try:
        # Determine if it's a status or plan filter
        if filter_lower in status_mapping:
            # Filter by account status
            db_users = await UserModel.find(
                UserModel.account_status == status_mapping[filter_lower],
                UserModel.role == UserRole.USER  # Exclude admins
            ).skip(skip).limit(limit).to_list()
            
        elif filter_lower in plan_mapping:
            # Filter by subscription plan
            db_users = await UserModel.find(
                UserModel.plan == plan_mapping[filter_lower],
                UserModel.role == UserRole.USER  # Exclude admins
            ).skip(skip).limit(limit).to_list()
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Invalid filter value: {filter_value}. Use: active, inactive, suspended, free, basic, or premium"
            )
        ...
        for db_user in db_users:
            ...
            # Get study time
            try:
                time_data = await TimeStorageModel.find_one(
                    TimeStorageModel.user_id == db_user.id
                )
                study_hours = time_data.total_time if time_data else 0
            except:
                study_hours = 0
            ...
            res.append({
                ...
                "email": db_user.email,
                "subscription": db_user.plan.value,  # ✅ Use .value for enum
                "account_status": db_user.account_status.value,  # ✅ Use .value
                "created_at": db_user.created_at.isoformat() if db_user.created_at else None,
                "average_score": round(success_rate, 2),
                "tests_completed": total_ans,
        ...
        pipeline = [
...

            {
                "$group": {
                    ...
                    "total_lessons": {"$first": "$total_lessons_count"},
...

            {
                "$project": {
                    ...
                    "course_completion": {
                        "$cond": [
                            {"$gt": ["$total_lessons", 0]},
                            {"$divide": ["$sum_progress", "$total_lessons"]},
                            0
                        ]
                    }
                }
...

            {
                "$group": {
                    ...
                    "total_students": {
                        "$sum": {"$cond": [{"$ifNull": ["$user_id", False]}, 1, 0]}
                    },
                    "completed_count": {
                        "$sum": {"$cond": [{"$gte": ["$course_completion", 99.9]}, 1, 0]}
                    },
                    "avg_progress": {"$avg": "$course_completion"}
                }
            },
            {
                "$project": {
...
@router.get("/statistics/subscription-distribution", status_code=status.HTTP_200_OK)
async def get_subscription_distribution():
    """
    Get subscription distribution statistics
    Returns count and percentage of users per plan
    """
    try:
        # Get total users count (excluding admins)
        total_users = await UserModel.find(
            UserModel.role == UserRole.USER
        ).count()
        ...
        collection = UserModel.get_motor_collection()
        results = await collection.aggregate(pipeline).to_list(None)
        
        # Format response with colors
        plan_colors = {
            "free": "#FFC0CB",      # Pink
            "basic": "#00008B",     # Dark Blue
            "premium": "#DC143C"    # Crimson
        }
        
        distribution = []
        for result in results:
            plan_name = result["_id"] or "free"
            count = result["count"]
            ...
            distribution.append({
                ...
                "color": plan_colors.get(plan_name.lower(), "#808080")
            })
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error fetching subscription distribution: {str(e)}"
        )